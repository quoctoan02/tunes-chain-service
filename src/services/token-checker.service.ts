...
export const mintNft = async (log_data: any, collection: any, log: any, blockchain: any, conn: any) => {
    // console.log("=>(token-checker.service.ts:93) log_data", log_data);
    //
    // const to_address = log_data.args.to.toLowerCase();
    // const from_address = log_data.args.from.toLowerCase();
    // console.log({ to_address });
...
export const buySong = async (log_data: any, collection: any, log: any, contract_list: any, conn: any) => {
    ...
    const user = await UserModel.getByType("address", from)
    if(user) {
        await UserSongController.buy({songId: buySongId, userId: user.id, price})
    }
};